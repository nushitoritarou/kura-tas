...
## 1. デザインコンセプト
**「Multi-Theme Ready & Default Eye-Friendly Color」**
将来的なテーマ切り替え機能（ダークモード等）を見据えた設計を前提としつつ、デフォルトのカラースキームとして、長時間の作業で目が疲れにくくエンジニアに馴染み深い「サクラエディタ」の配色にインスパイアされたカラーパレットを採用しています。

## 2. デフォルト・カラーパレット (Sakura Editor Inspired)
...
### 4.3. 操作フィードバック
- 右クリックで表示されるコンテキストメニューは `#F0F0F0` 背景に `#D7D7D7` のシャドウを付け、サクラエディタのメニューをオマージュしたデザインにする（デフォルトテーマ時）。
